...
# loading the data
df = pd.read_csv(
    'https://raw.githubusercontent.com/viveknunia/car_price/main/cars_price.csv')
...
auto = df[['condition', 'mileage(kilometers)', 'fuel_type', 'volume(cm3)', 'color',
           'transmission', 'drive_unit', 'segment', 'brand_avg_price', 'brand_category', 'priceUSD']]
price = df['priceUSD']
...
df_train, df_test = train_test_split(
    auto, train_size=0.8, test_size=0.2, random_state=100)
...
print(math.sqrt(mean_squared_error(
    df_pred_test['price'], df_test_ori['price'])), ' = root_mean_squared_error of the test data')
